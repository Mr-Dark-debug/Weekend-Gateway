...
class ChatService {
  ...
  Future<void> sendMessage(String tripId, String userId, String content) async {
    ...
    try {
      ...
      _logger.e(
        'Error sending message to trip $tripId by user $userId',
        e,
        stackTrace,
      );
  ...
  Future<List<UserModel>> getMessageAuthors(List<String> userIds) async {
    ...
    try {
      final response = await _supabase
          ...
          ) // Ensure all fields for UserModel.fromJson are selected
          .in_(
            'id',
            userIds.toSet().toList(),
          ); // Use toSet().toList() to ensure unique IDs

      return response.map((userData) => UserModel.fromJson(userData)).toList();
    } catch (e, stackTrace) {
      _logger.e(
        'Error fetching message authors for user IDs: $userIds',
        e,
        stackTrace,
      );
  ...
  Stream<List<TripMessageModel>> getTripMessagesStream(String tripId) {
    final StreamController<List<TripMessageModel>> controller =
        StreamController.broadcast();
    Map<String, UserModel> userCache = {}; // Cache for author details
    List<TripMessageModel> currentMessages =
        []; // Local list to manage messages
      ...
      try {
        ...
        if (historicalData.isNotEmpty) {
          ...
          currentMessages.addAll(
            historicalData.map((msgData) {
              return TripMessageModel.fromJson(
                msgData,
                authorDetails: userCache[msgData['user_id']],
              );
            }),
          );
        }
        ...
        _logger.e(
          'Error fetching initial messages for trip $tripId',
          e,
          stackTrace,
        );
    ...
    // TODO: Update this section based on the current Supabase Realtime API for listening to database changes.
    // The onPostgresChanges API might have been updated or replaced.
    ...
    // Placeholder for subscription object if the above block is commented out
    final RealtimeChannel subscription = _supabase.channel(
      'public:trip_messages:trip_id=eq.$tripId',
    );
    // Ensure the subscription is started if needed, or handle its absence gracefully.
    // For now, just creating the channel object to avoid breaking onCancel.
    // Actual subscription and event handling is commented out.
    subscription.subscribe((String status, {Object? error}) {
      if (status == 'SUBSCRIBED') {
        _logger.i(
          'Successfully subscribed to trip_messages channel for trip $tripId.',
        );
        // Potentially trigger a re-fetch if messages might have been missed between initial load and subscription
        // fetchAndListen(); // Or a more targeted fetch for recent messages
          ...
          status == 'CLOSED') {
        _logger.e(
          'Subscription error for trip $tripId: $status, Error: $error',
        );
        controller.addError('Subscription error: $status. Error: $error');
      }
    });
