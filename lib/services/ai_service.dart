...
import 'package:weekend_gateway/models/user_model.dart'; // Needed for author in TripModel.fromJson, though AI won't generate this directly
import 'package:uuid/uuid.dart'; // For generating IDs for new models

class AIService {
  // Using a placeholder URL as actual API integration is out of scope
  final String _mockApiUrl = 'https://api.example.com/generate_itinerary';
  final Uuid _uuid = Uuid();

  Future<String> generateItineraryRaw(Map<String, dynamic> preferences) async {
    // Construct a prompt (simplified for this mock)
    String prompt = "Generate a travel itinerary for a trip to ${preferences['destination']} "
    ...
    print("Sending mock request to AI with prompt: $prompt");
    ...
    if (preferences['destination'].toLowerCase().contains('paris')) {
      return Future.value(jsonEncode({
        ...
        "duration_days": preferences['duration'] ?? 3, // Use provided duration
        "is_public": false,
        "description": "An amazing ${preferences['duration'] ?? 3}-day trip to Paris, generated by AI, focusing on ${preferences['interests']}.",
        "cover_image_url": "https://images.unsplash.com/photo-1502602898657-3e91760cbb34?q=80&w=1200", // More generic Paris image
        "latitude": 48.8566,
        "longitude": 2.3522,
        ...
        "days": [
            ...
            "activities": [
              {
                ...
                "description": "Settle into your accommodation and take a moment to relax.",
                "location_name": "Your Hotel in Central Paris",
                "time": "14:00"
              },
              {
                "title": "Eiffel Tower Experience",
                "description": "Visit the iconic Eiffel Tower. Consider pre-booking tickets. Enjoy the views from the top.",
                "location_name": "Eiffel Tower, Champ de Mars, 5 Avenue Anatole France, 75007 Paris",
                "time": "16:00",
                "latitude": 48.8584,
                "longitude": 2.2945
              },
              {
                ...
                "description": "Enjoy a relaxing evening cruise on the Seine River to see illuminated monuments.",
          ...
          },
          {
            ...
            "activities": [
              {
                ...
                "description": "Explore masterpieces like the Mona Lisa and Venus de Milo. Allocate several hours.",
              ...
              },
              {
                ...
                "description": "Enjoy lunch in the trendy Le Marais district, known for its historic sites and vibrant atmosphere.",
                "location_name": "Le Marais District",
                "time": "13:30"
              },
              {
                "title": "Explore Montmartre & Sacré-Cœur",
                "description": "Wander through the artistic streets of Montmartre and visit the Sacré-Cœur Basilica for panoramic city views.",
           ...
           if ((preferences['duration'] ?? 3) >= 3) { // Add day 3 if duration allows
            "day_number": 3,
            "title": "Palace & Departure",
            "activities": [
              {
                "title": "Versailles Palace & Gardens (Optional Half-Day)",
                "description": "Consider a trip to the magnificent Palace of Versailles. Requires travel time.",
                "location_name": "Palace of Versailles, Place d'Armes, 78000 Versailles",
                "time": "09:30",
                "latitude": 48.8049,
                "longitude": 2.1204
              },
              {
                ...
                "description": "Enjoy some last-minute shopping or a Parisian café before heading to the airport/station.",
      ...
      // Generic fallback mock response
       return Future.value(jsonEncode({
        ...
        "is_public": false,
        "description": "A wonderful ${preferences['duration'] ?? 2}-day trip to ${preferences['destination']} based on your interests in ${preferences['interests']}.",
        "cover_image_url": "https://images.unsplash.com/photo-1503220317375-aaad61436b1b?q=80&w=1200", // Generic travel image
        "latitude": null, // Could be fetched or left null
        "longitude": null,
        "price_level": _getMockPriceLevel(preferences['budget']),
        "days": List.generate(preferences['duration'] ?? 2, (index) => {
          "day_number": index + 1,
          "title": "Day ${index + 1}: Exploring ${preferences['destination']}",
          "activities": [
            {
              "title": "Morning Exploration based on ${preferences['interests']}",
              "description": "Discover local attractions aligning with your interests.",
            ...
            },
            {
              ...
              "description": "Engage in an activity related to ${preferences['interests']}.",
              "location_name": "Specific venue in ${preferences['destination']}",
              "time": "15:00"
            }
          ]
        })
  ...
  TripModel? parseApiResponse(String rawJson, String currentUserId) {
    try {
            ...
            if (dayData['activities'] is List) {
              for (var activityData in (dayData['activities'] as List)) {
                if (activityData is Map<String, dynamic>) {
                  activities.add(TripActivityModel(
                    id: _uuid.v4(), // Generate new ID
                    tripDayId: '', // This will be set when day is created, or handled by backend
                    title: activityData['title'] ?? 'Unnamed Activity',
                    description: activityData['description'],
                    location: activityData['location_name'], // Map location_name to location
                    time: activityData['time'],
                    latitude: (activityData['latitude'] as num?)?.toDouble(),
                    longitude: (activityData['longitude'] as num?)?.toDouble(),
                    createdAt: DateTime.now(),
                    updatedAt: DateTime.now(),
                    photoUrls: [], // AI won't generate photos
                  ));
                }
              }
            }
      ...
      return TripModel(
        id: _uuid.v4(), // Generate new ID, will be overridden by Supabase on insert
        ...
        thumbnailUrl: jsonResponse['cover_image_url'], // Use cover as thumbnail initially
        isPublic: jsonResponse['is_public'] ?? false, // Default to private
        userId: currentUserId, // Set current user as owner
        createdAt: DateTime.now(),
        updatedAt: DateTime.now(),
        author: null, // Author details not generated by AI, will be set if needed by app logic
        tripDays: tripDays,
        comments: [],
        likeCount: 0,
        ratingCount: 0,
        ...
        userRating: null,
        latitude: (jsonResponse['latitude'] as num?)?.toDouble(),
        longitude: (jsonResponse['longitude'] as num?)?.toDouble(),
      );
    } catch (e) {
